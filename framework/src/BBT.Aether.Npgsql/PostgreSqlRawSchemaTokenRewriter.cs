...
internal static class PostgreSqlRawSchemaTokenRewriter
{
    ...
    public static RewriteResult Rewrite(string commandText, string? replacement)
        => RewriteCodeRegions(commandText, replacement, includeRawTokens: true);

    /// <summary>
    /// Rewrites EF model schema markers only in PostgreSQL code regions. The exact quoted marker
    /// is itself an identifier generated by EF and is therefore handled before generic quoted
    /// identifier protection; marker text embedded in any other quoted identifier is preserved.
    /// </summary>
    public static RewriteResult RewriteModelPlaceholder(string commandText, string replacement)
        => RewriteCodeRegions(commandText, replacement, includeRawTokens: false);

    private static RewriteResult RewriteCodeRegions(
        string commandText,
        string? replacement,
        bool includeRawTokens)
    {
        ...
        {
            var tokenLength = includeRawTokens
                ? GetTokenLength(commandText, position)
                : GetModelTokenLength(commandText, position);
            if (tokenLength > 0)
            {
                foundToken = true;
    ...
    private static int GetModelTokenLength(string sql, int position)
    {
        if (StartsWith(sql, position, AetherSchemaModel.QuotedPlaceholder))
            return AetherSchemaModel.QuotedPlaceholder.Length;
        if (StartsWith(sql, position, AetherSchemaModel.Placeholder))
            return AetherSchemaModel.Placeholder.Length;
        return 0;
    }
