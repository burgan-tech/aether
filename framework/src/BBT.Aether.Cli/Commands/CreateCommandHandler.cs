...
public class CreateCommandHandler(IConsole console) : ICommandHandler<CreateCommandOptions, CreateCommandArguments>
{
    ...
    private const string RepoUrl = "https://github.com/burgan-tech/aether-template.git";
        ...
        CancellationToken cancellationToken)
    {
        ...
        console.WriteLine($"Company/Team Name: {options.Team}");
        ...
        try
        {
            RunSolutionRenamer(tempDir, options.Team, arguments!.Name, options.Output);
    ...
    private void RunSolutionRenamer(string tempDir, string companyName, string projectName, string outputPath)
    {
        var templatePath = Path.Combine(tempDir, "templates", "api");
        if (string.IsNullOrEmpty(companyName))
        {
            companyName = CliConstants.DefaultCompanyName;
        }

        var solutionRenamer = new SolutionRenamer(
            templatePath, "BBT", "MyProjectName", companyName, projectName);
